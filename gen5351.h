   ...
   public:
    ...
    void init() {
        ...
        si5351->drive_strength(
            SI5351_CLK0,
            SI5351_DRIVE_2MA);  // Output current 2MA, 4MA, 6MA or 8MA
    ...
    void updateEnabled() {
        if (enabled) {
            ...
            si5351->output_enable(SI5351_CLK0,
                                  0);  // 1 - Enable / 0 - Disable CLK
        }
    }

    void showFreq() override { genBase::showFreq(); }
    ...
    const char *name() override { return "si5331"; }

   private:
    Si5351 *si5351;
    uint8_t i2c_addr = SI5351_BUS_BASE_ADDR;
    long interfreq =
        0;  // Enter your IF frequency, ex: 455 = 455kHz, 10700 = 10.7MHz, 0 =
            // to direct convert receiver or RF generator, + will add and - will
            // subtract IF offfset.
    long cal = 33000;  // Si5351 calibration factor, adjust to get exatcly
                       // 10MHz. Increasing this value will decreases the
